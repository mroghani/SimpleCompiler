...
# if 201103L <= YY_CPLUSPLUS
  ...
  private:
    ...
    union union_type
    {
      ...
      // params
      // paramList
      char dummy5[sizeof (std::vector<Var>)];
    };
    ...
    TOK_IF = 3,                    // "if"
    TOK_ELSE = 4,                  // "else"
    TOK_ELSE_IF = 5,               // "elseif"
    TOK_WHILE = 6,                 // "while"
    TOK_CONTINUE = 7,              // "continue"
    TOK_BREAK = 8,                 // "break"
    TOK_FOR = 9,                   // "for"
    ...
    TOK_NOT = 22,                  // "!"
    TOK_LESSTHAN = 23,             // "<"
    TOK_LESSTHANEQUAL = 24,        // "<="
    TOK_EQUAL = 25,                // "=="
    TOK_NOTEQUAL = 26,             // "!="
    TOK_GREATERTHAN = 27,          // ">"
    TOK_GREATERTHANEQUAL = 28,     // ">="
    TOK_LPAREN = 29,               // "("
    TOK_RPAREN = 30,               // ")"
    TOK_LBRACK = 31,               // "["
    TOK_RBRACK = 32,               // "]"
    ...
    TOK_CHAR = 38,                 // "char"
    TOK_VOID = 39,                 // "void"
    TOK_INTCONST = 40,             // "int constant"
    TOK_CHARCONST = 41,            // "char constant"
    TOK_ID = 42                    // "identifier"
      };
    ...
    struct symbol_kind
    {
      enum symbol_kind_type
      {
        ...
        S_CHAR = 38,                             // "char"
        S_VOID = 39,                             // "void"
        S_INTCONST = 40,                         // "int constant"
        S_CHARCONST = 41,                        // "char constant"
        S_ID = 42,                               // "identifier"
        S_YYACCEPT = 43,                         // $accept
        S_unit = 44,                             // unit
        S_programm = 45,                         // programm
        S_declLists = 46,                        // declLists
        S_decl = 47,                             // decl
        S_varDecl = 48,                          // varDecl
        S_varType = 49,                          // varType
        S_funcDecl = 50,                         // funcDecl
        S_51_1 = 51,                             // $@1
        S_52_2 = 52,                             // $@2
        S_53_3 = 53,                             // $@3
        S_54_4 = 54,                             // @4
        S_params = 55,                           // params
        S_paramList = 56,                        // paramList
        S_paramItem = 57,                        // paramItem
        S_stmtList = 58,                         // stmtList
        S_stmt = 59,                             // stmt
        S_selectStmt = 60,                       // selectStmt
        S_iterStmt = 61,                         // iterStmt
        S_whileStmt = 62,                        // whileStmt
        S_forStmt = 63,                          // forStmt
        S_breakStmt = 64,                        // breakStmt
        S_continueStmt = 65,                     // continueStmt
        S_returnStmt = 66,                       // returnStmt
        S_expStmt = 67,                          // expStmt
        S_exp = 68,                              // exp
        S_simpleExp = 69,                        // simpleExp
        S_andExp = 70,                           // andExp
        S_unaryRelExp = 71,                      // unaryRelExp
        S_relExp = 72,                           // relExp
        S_relop = 73,                            // relop
        S_sumExp = 74,                           // sumExp
        S_sumop = 75,                            // sumop
        S_mulExp = 76,                           // mulExp
        S_mulop = 77,                            // mulop
        S_unaryExp = 78,                         // unaryExp
        S_unaryop = 79,                          // unaryop
        S_factor = 80,                           // factor
        S_mutable = 81,                          // mutable
        S_immutable = 82,                        // immutable
        S_call = 83,                             // call
        S_args = 84,                             // args
        S_argList = 85,                          // argList
        S_constant = 86                          // constant
      };
    };
        ...
        switch (this->kind ())
    {
      ...
      case symbol_kind::S_paramItem: // paramItem
        value.move< Var > (std::move (that.value));
        break;
      ...
      case symbol_kind::S_paramList: // paramList
        value.move< std::vector<Var> > (std::move (that.value));
        break;
...
#if 201103L <= YY_CPLUSPLUS
      basic_symbol (typename Base::kind_type t, Var&& v, location_type&& l)
        : Base (t)
        , value (std::move (v))
        , location (std::move (l))
      {}
#else
      basic_symbol (typename Base::kind_type t, const Var& v, const location_type& l)
        : Base (t)
        , value (v)
        , location (l)
      {}
...
#if 201103L <= YY_CPLUSPLUS
      basic_symbol (typename Base::kind_type t, std::vector<Var>&& v, location_type&& l)
        : Base (t)
        , value (std::move (v))
        , location (std::move (l))
      {}
#else
      basic_symbol (typename Base::kind_type t, const std::vector<Var>& v, const location_type& l)
        : Base (t)
        , value (v)
        , location (l)
      {}
...
switch (yykind)
    {
      ...
      case symbol_kind::S_params: // params
      case symbol_kind::S_paramList: // paramList
        value.template destroy< std::vector<Var> > ();
        break;
...
#endif
      {
        YY_ASSERT ((token::TOK_INTCONST <= tok && tok <= token::TOK_CHARCONST));
      }
#if 201103L <= YY_CPLUSPLUS
    ...
    // Implementation of make_symbol for each symbol type.
      ...
      {
        return symbol_type (token::TOK_ELSE_IF, std::move (l));
      }
#else
      static
      symbol_type
      ...
      {
        return symbol_type (token::TOK_ELSE_IF, l);
      }
#endif
#if 201103L <= YY_CPLUSPLUS
      static
      ...
      static
      symbol_type
      make_LCBRACK (location_type l)
      {
        return symbol_type (token::TOK_LCBRACK, std::move (l));
      }
#else
      static
      symbol_type
      make_LCBRACK (const location_type& l)
      {
        return symbol_type (token::TOK_LCBRACK, l);
      }
#endif
#if 201103L <= YY_CPLUSPLUS
      static
      symbol_type
      make_RCBRACK (location_type l)
      {
        return symbol_type (token::TOK_RCBRACK, std::move (l));
      }
#else
      static
      symbol_type
      make_RCBRACK (const location_type& l)
      {
        return symbol_type (token::TOK_RCBRACK, l);
      }
#endif
#if 201103L <= YY_CPLUSPLUS
      static
      ...
      static
      symbol_type
      make_CHARCONST (int v, location_type l)
      {
        return symbol_type (token::TOK_CHARCONST, std::move (v), std::move (l));
      }
#else
      static
      symbol_type
      make_CHARCONST (const int& v, const location_type& l)
      {
        return symbol_type (token::TOK_CHARCONST, v, l);
      }
#endif
#if 201103L <= YY_CPLUSPLUS
      static
    ...
    , location (that.location)
  {
    switch (this->kind ())
    {
      ...
      case symbol_kind::S_paramItem: // paramItem
        value.copy< Var > (YY_MOVE (that.value));
        break;
      ...
      case symbol_kind::S_params: // params
      case symbol_kind::S_paramList: // paramList
        value.copy< std::vector<Var> > (YY_MOVE (that.value));
        break;
  ...
  parser::basic_symbol<Base>::move (basic_symbol& s)
  {
    ...
    switch (this->kind ())
    {
      ...
      case symbol_kind::S_paramItem: // paramItem
        value.move< Var > (YY_MOVE (s.value));
        break;
      ...
      case symbol_kind::S_params: // params
      case symbol_kind::S_paramList: // paramList
        value.move< std::vector<Var> > (YY_MOVE (s.value));
        break;
